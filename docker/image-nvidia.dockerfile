...
RUN DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends -o Dpkg::Options::="--force-confnew" \
                    liblapack-dev liblapack3 libopenblas-base libopenblas-dev libgfortran-7-dev avahi-daemon libnss-mdns
...
RUN mkdir /smart_app && chown ${myuser}:${mygroup} -R /smart_app
WORKDIR /smart_app
